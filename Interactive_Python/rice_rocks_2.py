...
class Ship:
    ...
    def __init__(self, pos, vel, angle, image, info, fire, hull, player):
        ...
        self._fire = False
        self._hull = hull    # the hull is equivalent to the number of lives
        self._player = player
        self._shield = 180    # the ship is spawned with the shield on
        self._score = 0
        self._parent = None
    ...
    def update_angle_vel(self, a_vel):
        ...
        self._angle_vel += a_vel

    def draw(self,canvas):
        """Draws the ship in the canvas"""
        if self._thrust:    # draws the ship with burners on
            ...
            canvas.draw_image(self._image,
                              ...
                              self._image_size,
                              self._angle)
        if self._shield > 0:    # if the shield is active, draws a semitransparent color cylcling circle around the ship
            color = 'hsla(' + str((time + 20 * self._player) * 6 % 360) + ', 50%, 80%, 0.3)'
            canvas.draw_circle(self._pos, self._radius + 15, 6, color, color)
    ...
    def update(self):
        ...
        if started:    # If a game is ongoing
            ...
            self._angle += self._angle_vel    # rotates the ship
            for d in [0, 1]:
                if self._thrust:
                    self._vel[d] += THRUST_POWER * angle_to_vector(self._angle)[d]    # accelerates the ship in the direction it's facing
                self._pos[d] += self._vel[d]    # displaces ship
                self._vel[d] *= .99    # "friction" term to eventually stop the ship if uncomanded

            # Shooting missiles
            if self._fire and (time % RATE_OF_FIRE == 0):    # While the fire button is pressed, the ship is continuelly shooting.
                am_pos = [.0, .0]
                am_vel = [.0, .0]
                if (time // RATE_OF_FIRE) % 2 == 0:    # alternates left and right gun
                    ...
                    gun = -1
                if self._player == 0:    # draws a different missile icon for each player
                ...
                for dim in [0, 1]:    # gets the parameters of position, angle and velocity for the missile and creates it
                    am_pos[dim] = self._pos[dim] + .8 * self._radius * angle_to_vector(self._angle + gun * .7)[dim]
                    am_vel[dim] = self._vel[dim] + MISSILE_VEL * angle_to_vector(self._angle)[dim]
            ...
            if self._shield > 0: self._shield -= 1    # if the shield is active, decrease its time counter

            # Toroidal space: warping the ship through the edges of the screen
            if self._pos[0] < -self._radius:    # wraps the objects around the edges. I choose to leave the object dissapear completely in one side before spawning it in the other corner of the screen
                self._pos[0] = WIDTH + self._radius
            if self._pos[0] > WIDTH + self._radius:
                self._pos[0] = -self._radius
            if self._pos[1] < -self._radius:
                self._pos[1] = HEIGHT + self._radius
            if self._pos[1] > HEIGHT + self._radius:
                self._pos[1] = -self._radius

            # Updating the score
            score[self._player] = self._score    # updates the gobal score variable to reflect the current player score
...
class Sprite:
    ...
    def __init__(self, pos, vel, ang, ang_vel, image, info, sound = None, parent = None):
        ...
        self._columns = info.get_columns()
        self._rows = info.get_rows()
        self._age = 0
        self._parent = parent    # tracks who fired a missile
        self._collided = False
        if sound:
            sound.rewind()
            sound.play()
    ...
    def draw(self, canvas):
        ...
        current_i = ((self._age / 4) % self._columns) // 1
        current_j = ((self._age / 4) % self._rows) // 1
        canvas.draw_image(self._image,
                          [self._image_center[0] + current_i * self._image_size[0], self._image_center[1] + current_j * self._image_size[0]],
                          self._image_size,
                          self._pos,
                          self._image_size,
                          self._angle)
...
def keydown(key):
    ...
    if started:
        if not game_mode == 0:    # player 1 controls
            if key == pygame.KEY_MAP['a']:
                player1.update_angle_vel(-ANGLE_VEL_MODULE)
            elif key == pygame.KEY_MAP['d']:
                player1.update_angle_vel(ANGLE_VEL_MODULE)
            elif key == pygame.KEY_MAP['w']:
                player1.ship_thrust(True)
            elif key == pygame.KEY_MAP['space']:
                player1.fire = True
        if game_mode > 1:    # player 2 controls
            if key == pygame.KEY_MAP['left']:
                player2.update_angle_vel(-ANGLE_VEL_MODULE)
            elif key == pygame.KEY_MAP['right']:
                player2.update_angle_vel(ANGLE_VEL_MODULE)
            elif key == pygame.KEY_MAP['up']:
...
debris_image = pygame.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/debris1_brown.png")
...
nebula_image = pygame.load_image("http://i.imgur.com/5K8jCUJ.png")

# splash image
splash_info = ImageInfo([200, 150], [400, 300])
splash_image = pygame.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/splash.png")
...
soundtrack = pygame.load_sound("http://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/soundtrack.mp3")
missile_sound = pygame.load_sound("http://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/missile.mp3")
missile_sound.set_volume(.5)
ship_thrust_sound = pygame.load_sound("http://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/thrust.mp3")
explosion_sound = pygame.load_sound("http://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/explosion.mp3")


# initialize frame and register handlers
frame = pygame.create_frame("My Rice Rocks", WIDTH, HEIGHT)
...
rock_timer = pygame.create_timer(250.0, rock_spawner)
